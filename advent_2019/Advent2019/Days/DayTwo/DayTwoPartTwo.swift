...
class DayTwoPartTwo: DayTwo {
    ...
    override func description() -> String {
        return "What is 100 * noun + verb?"
    }
    
    override func answer() -> DayAnswer {
        
        let solution = solveProblem()
        let alert = UIAlertController(title: "Answer for \(partTitle())", message: "100 * noun + verb is \(solution)", preferredStyle: .alert)
...
private extension DayTwoPartTwo {
    ...
    func solveProblem() -> Int {
        let answerCode = parseAndSplitData()

        for noun in 0...99 {
            for verb in 0...99 {
                if determinePositionZero(from: answerCode, noun: noun, verb: verb) == 19690720 {
